...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf) {
  ...
  while(addr_copy < finish){
      ...
      jbod_operation(op4, block);
      uint32_t op5 = use_addr(JBOD_SEEK_TO_BLOCK, 0, num_for_block);
      jbod_operation(op5, block);
      uint32_t op6 = use_addr(JBOD_WRITE_BLOCK, 0, 0);
      jbod_operation(op6, temporaryBuf);


      // Cache implementation for write, checking for cache enabled and then lookup for = to 1, then updating otherwise simply insert
      if (cache_enabled() == true) {
        if (cache_insert(dB, num_for_block, temporaryBuf) == -1){
          cache_update(dB, num_for_block, temporaryBuf);
        }
      }
